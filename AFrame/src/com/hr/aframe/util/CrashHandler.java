 ...
 * 
 * 
 * UncaughtExceptionHandler：线程未捕获异常控制器是用来处理未捕获异常的。<br>
 * 如果程序出现了未捕获异常默认情况下则会出现强行关闭对话框<br>
 * 实现该接口并注册为程序中的默认未捕获异常处理 ，这样当未捕获异常发生时，就可以做些异常处理操作 例如：收集异常信息，发送错误报告 等。
 * <p>
 * <h1>使用提示:</h1>
 * 1，获取实例对象：CrashHandler crashHandler = CrashHandler.getInstance();<br>
 * 2，注册crashHandler：crashHandler.registerHandler(mContext);<br>
 * 3，设置向服务器提交方式： setSubmitServerMethodSetting(xx);<br>
 * 4，发送以前没发送的错误日志(可选)：crashHandler.sendPreviousReportsToServer();
 */
public class CrashHandler implements UncaughtExceptionHandler {
	...
	/**
	 * properties 的KEY和value必须为 String 类型，如果不是String
	 * 类型将会抛出ClassCastException异常，请注意这点。
	 */
	...
	private boolean handleException(Throwable ex) {
		...
		new Thread() {
			...
			public void run() {
				...
				Looper.prepare();
				Toast.makeText(mContext, "(┬＿┬)程序出错啦:" + msg, Toast.LENGTH_LONG)
						.show();
				Looper.loop();
			}
		}.start();
	...
	public void collectCrashDeviceInfo(Context ctx) {
		try {
			...
			if (pi != null) {
				...
				// key and value must is String type
	...
	/**
	 * 保存错误信息到文件中 SharedPreferences将XML文件保存在/data/data/<package
	 * name>/shared_prefs目录下，
	 * 
	 * openFileOutput方法将文件保存在/data/data/<package name>/files目录下
	 * 
	 * SQLite数据库文件存放在/data/data/<package name>/databases目录下
	 * 
	 * @param ex
	 * @return
	 */
